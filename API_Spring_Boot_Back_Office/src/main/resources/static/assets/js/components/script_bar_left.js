//updatingBarChart.js
var arcColorFn	= d3.interpolateHsl(d3.rgb('#ff0000'), d3.rgb('#04ff41'));
...
var redrawChart = function(targetID, newdata) {
	...
	newRow.append("text")
	  ...
	  .attr("opacity",0)
	  .attr("dy",".35em")
	  .attr("dx",function(d){return(Math.min(-x(d.frequence+3),-x(17)))})
	  ...
	  .attr("opacity",0)
	  .attr("dy",".35em")
	  .attr("dx",function(d){return(Math.min(-x(d.frequence+4),-x(18)))})
	  .text('|'); 
	  ...
	  .attr("opacity",0)
	  .attr("dy",".35em")
	  .attr("dx",function(d){return(Math.min(-x(d.frequence+5),-x(19)))})
	  ...
	  .attr('font-size', '20px')
	  .attr("dx",function(d){return(Math.min(-x(d.frequence+7),-x(21)))})
	  .attr("dy",".35em")
	...
	chartRow.transition()
		.delay(delay)
		.duration(900)
		.attr("transform", function(d){ return "translate(580," + y(d.topic) + ")"; });
};
...
function chart_positive_polarity(){
	var redrawChart = function(targetID, newdata) {
        ...
        newRow.append("text")
          ...
          .attr("opacity",0)
          .attr("dy",".35em")
          .attr("dx",function(d){return(Math.min(-x(d.frequence+7),-x(27)))})
          .text(function(d){return d.frequence;}); 
          newRow.append("text")
          .attr("class","label3")
          .attr("y", y.rangeBand()/2)
