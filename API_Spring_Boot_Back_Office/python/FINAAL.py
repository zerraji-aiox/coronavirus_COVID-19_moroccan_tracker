...
def Calcule_taux(S,A):
    ...
    for j in range(1,31):
        ...
        confirmed = colonne_confirmed_maroc[j-1]*(1+cof[ index-1 + j-1])
    ...
    for i in range(31):
        ...
        nb_cas_estime.append(np.floor (true_cases(colonne_confirmed_maroc[i]*0.039, 6.7, day ,fatality_rate=0.87,days_to_death=17.3)))
    ...
    for j in range(len(Region)): 
            ...
            text='['
            for i in range(31):
                confirmed = colonne_confirmed_maroc[i]*(1+cof[index + i])*(b/a)
                b=confirmed
                a=colonne_confirmed_maroc[i]
                if i==0 :
                        text+='{"date":"'+colonne_date[i]+'","nb_cas_confirme":'+str(A[j])+',"increase_rate":'+str(colonne_Rate_Increase[i])+',"nb_cas_estime":'+str((b/a)*nb_cas_estime[i])+'},\n'
                else:
                    if i == 30:
